...
public class HomeServiceImpl {
    ...
    public ResponseResult selectPubicData() {
        ...
        //获取精品分类列表
        List<Category> categories = categoryMapper.selectListByHome();
        //获取首页轮播
        List<SystemArticleListVO> articles = articleMapper.selectListByBanner();


        return ResponseResult.success().putExtra("categories",categories).putExtra("articles",articles);
    }
    ...
    public ResponseResult getWebSiteInfo() {
        ...
        return ResponseResult.success()
                ...
                .putExtra("tagCount",tagCount).putExtra("newArticleList",apiArticleListVOS).putExtra("tagCloud",tags)
                .putExtra("hotArticles",hotArticles).putExtra("webConfig",webConfig);
    }
}
