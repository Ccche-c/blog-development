...
public class AliUploadStrategyImpl implements FileUploadStrategy {
    ...
    /**
     * 获取所有文件 暂不进行分页（调用该会按照Put类请求次数计费，所以按需引入）
     * @param pageNo 页码
     * @param limit 页数
     * @return
     */
    ...
    public ResponseResult fileList(int pageNo, int limit) {
        ...
        ListObjectsRequest listObjectsRequest = new ListObjectsRequest(ali_bucket);
        ObjectListing objectListing = ossClient.listObjects(listObjectsRequest);
        return ResponseResult.success(objectListing);
    }
    ...
    public String fileUpload(MultipartFile file,String suffix) {
        ...
        try {
            ...
            return  "https://" + ali_bucket + "." + ali_endpoint + "/" + newFileName;
        } catch (IOException e) {
            e.printStackTrace();
        }finally {
            if (ossClient != null) {
    ...
    private OSS getOssClient() {
        // 创建OSS实例
        return new OSSClientBuilder().build(ali_endpoint, ali_accessKey, ali_secretKey);

        //添加自定义的配置参数创建
//        return new OSSClientBuilder().build(ali_endpoint, ali_accessKey, ali_secretKey,getClientBuilderConfiguration());
    }
